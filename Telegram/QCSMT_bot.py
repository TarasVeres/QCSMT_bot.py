...
from run_json import checker, update_sheet, skl_hum, ve_chat_skl
import writer
...
async def start(message: types.Message):
    ...
    if message.chat.id == message.from_user.id:
        ...
        if str(message.from_user.id) in Sheet['access_id']:
            Reply[m_id] = dict()
            Reply[m_id]['id_loca'] = Sheet['id_loca'][str(message.chat.id)]
...
async def device_callback(call: types.CallbackQuery):
    global mq, Reply, Sheet
    mq = ''
    button = types.InlineKeyboardMarkup(row_width=1)
    c_id = call.from_user.id
    try:
        Reply[c_id]['reported'] = Sheet['access_id'][str(c_id)]
        Reply[c_id]['back'] = 'back'
        if call.data in 'back':
            ...
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            line = [i for i in Sheet[skl_hum(Reply[c_id], 'lines')]]
            button = Inline_Keyboard.inline_c2(line, 'back', Reply[c_id])
            if call.data in 'break':
                await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
            ...
            await call.answer('')
            Reply[c_id]['yes_photo'] = 'defects_1'
            delete = ['count_defects', 'data', 'defects', 'device', 'line', 'no_amount_data', 'non_liquidity', 'yes_coments_information'
                      'non_liquidity_board', 'non_liquidity_mult', 'project', 'side', 'spec', 'stack_defects', 'team'
        ...
        elif (call.data in Sheet[skl_hum(Reply[c_id], 'lines')]) and ('break' in Reply[c_id]):
            await call.answer('')
            Reply[c_id]['line'] = call.data
            ...
            button = Inline_Keyboard.inline_c2(time_break, 'break', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть перерву:', reply_markup=button)
        elif call.data in Sheet['break']:
            await call.answer('')
            Reply[c_id]['time_break'] = Sheet['break'][call.data]
            ...
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            post = assembler_message.assembling_break(Reply[c_id])
            button.add(types.InlineKeyboardButton(text='Так', callback_data='yes_break'),
                       types.InlineKeyboardButton(text='⬅️Назад', callback_data=Reply[c_id]['line']))
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
            ...
            if 'yes_break' in call.data:
                ...
                reply = assembler_message.assembling_break(Reply[c_id])
                await bot.send_message(chat_id=ve_chat_skl(Reply[c_id]), text=reply)
            else:
                ...
                await bot.send_media_group(chat_id=ve_chat_skl(Reply[c_id]), media=Reply[c_id]['med'])
            ...
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть лінію:', reply_markup=button)
        elif (call.data in Sheet[skl_hum(Reply[c_id], 'lines')]) and (('yes_photo' in Reply[c_id]) or ('data' in Reply[c_id])):
            await call.answer('')
            Reply[c_id]['line'] = call.data
            delete = ['count_defects', 'defects', 'device', 'no_amount_data', 'non_liquidity', 'yes_coments_information'
                      'non_liquidity_board', 'non_liquidity_mult', 'project', 'side', 'spec', 'stack_defects', 'team'
                       'type', 'type_spec', 'writer_stack', '3_5_10', 'type_defect', 'coments']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            teams = [i for i in Sheet[skl_hum(Reply[c_id], 'teams')]]
            button = Inline_Keyboard.inline_c2_nonBacker(teams, 'backer_line')
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть команду:', reply_markup=button)
        elif (call.data in Sheet[skl_hum(Reply[c_id], 'teams')]) and (('yes_photo' in Reply[c_id]) or ('data' in Reply[c_id])):
            await call.answer('')
            Reply[c_id]['team'] = call.data
            delete = ['count_defects', 'defects', 'device', 'no_amount_data', 'non_liquidity', 'yes_coments_information'
                      'non_liquidity_board', 'non_liquidity_mult', 'project', 'side', 'spec', 'stack_defects',
                       'type', 'type_spec', 'writer_stack', '3_5_10', 'type_defect', 'coments']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            type = [i for i in Sheet[skl_hum(Reply[c_id], 'project_all')]]
            button = Inline_Keyboard.inline_c2(type, 'line', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть лінійку девайсів:', reply_markup=button)
        elif call.data in Sheet[skl_hum(Reply[c_id], 'project_all')]:
            await call.answer('')
            Reply[c_id]['type'] = call.data
            ...
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            device = [i for i in Sheet[skl_hum(Reply[c_id], 'project_all')][Reply[c_id]['type']]]
            button = Inline_Keyboard.inline_c2(device, 'team', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть девайс:', reply_markup=button)
        elif ('type' in Reply[c_id]) and (call.data in Sheet[skl_hum(Reply[c_id], 'project_all')][Reply[c_id]['type']]):
            await call.answer('')
            ...
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            project = [i for i in Sheet[skl_hum(Reply[c_id], 'project_all')][Reply[c_id]['type']][Reply[c_id]['device']]]
            button = Inline_Keyboard.inline_c1(project, 'type', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть проект:', reply_markup=button)
        elif (call.data in Sheet[skl_hum(Reply[c_id], 'project_all')][Reply[c_id]['type']][Reply[c_id]['device']]) \
                and ('data' in Reply[c_id]):
            await call.answer('')
            ...
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            side = [i for i in Sheet['spec_all'][Reply[c_id]['project']]]
            button = Inline_Keyboard.inline_c2(side, 'device', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть строну:', reply_markup=button)
        elif ('project' in Reply[c_id]) and (call.data in Sheet['spec_all'][Reply[c_id]['project']]):
            await call.answer('')
            Reply[c_id]['side'] = call.data
            Reply[c_id]['stack_defects'] = ''
            Reply[c_id]['writer_stack'] = []
            ...
            button.add(types.InlineKeyboardButton(text='⬅️Назад', callback_data=Reply[c_id]['project']))
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Внести кількість перевірених плат??', reply_markup=button)
        elif 'no_amount_data' in call.data:
            await call.answer('')
            Reply[c_id]['no_amount_data'] = call.data
            ...
            button = Inline_Keyboard.inline_c2(defects, 'side', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть дефект:', reply_markup=button)
        elif call.data in Sheet['defects']:
            await call.answer('')
            Reply[c_id]['defects'] = call.data
            delete = ['count_defects', 'non_liquidity', 'non_liquidity_board', 'non_liquidity_mult', 'spec',
                    'type_spec', '3_5_10', 'type_defect', 'coments', 'yes_coments_information']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            type_spec = [i for i in Sheet['spec_all'][Reply[c_id]['project']][Reply[c_id]['side']]]
            button = Inline_Keyboard.inline_c2_nonBacker(type_spec, 'no_amount_data')
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть тип компоненту:', reply_markup=button)
        elif ((('project' in Reply[c_id]) and ('side' in Reply[c_id])) and
                (call.data in Sheet['spec_all'][Reply[c_id]['project']][Reply[c_id]['side']])) and (call.data not in 'AZ'):
            await call.answer('')
            Reply[c_id]['type_spec'] = call.data
            delete = ['count_defects', 'non_liquidity', 'non_liquidity_board', 'non_liquidity_mult', 'spec',
                     '3_5_10', 'type_defect', 'coments', 'yes_coments_information']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            spec = [i for i in Sheet['spec_all'][Reply[c_id]['project']][Reply[c_id]['side']] \
                [Reply[c_id]['type_spec']]]
            if len(spec) > 99:
                Reply[c_id]['data_set_count'] = 0
                button, Reply[c_id]['data_set'] = Inline_Keyboard.inline_c3_data100(spec, 'defects', Reply[c_id], Reply[c_id]['data_set_count'])
            else:
                button = Inline_Keyboard.inline_c3(spec, 'defects', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Оберіть компонент:', reply_markup=button)
        elif 'data_set' in call.data:
            await call.answer('')
            if 'data_set+1' in call.data:
                Reply[c_id]['data_set_count'] += 1
                spec = [i for i in Sheet['spec_all'][Reply[c_id]['project']][Reply[c_id]['side']] \
                    [Reply[c_id]['type_spec']]]
                button, Reply[c_id]['data_set'] = Inline_Keyboard.inline_c3_data100(spec, 'defects', Reply[c_id], Reply[c_id]['data_set_count'])
            ...
            await call.answer('')
            Reply[c_id]['yes_amount_data'] = call.data
            delete = ['count_defects', 'defects', 'non_liquidity', 'non_liquidity_board', 'non_liquidity_mult', 'spec',
                      'type_spec', '3_5_10', 'type_defect', 'coments', 'yes_coments_information', 'yes_count_amount_data']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            button.add(types.InlineKeyboardButton(text='⬅️Назад', callback_data=Reply[c_id]['side']))
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Введіть кількість:', reply_markup=button)
        elif ((('project' in Reply[c_id]) and ('side' in Reply[c_id]) and ('type_spec' in Reply[c_id]))
            and (call.data in Sheet['spec_all'][Reply[c_id]['project']][Reply[c_id]['side']]
            [Reply[c_id]['type_spec']])) or (call.data in 'AZ'):
            await call.answer('')
            Reply[c_id]['spec'] = call.data
            ...
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            backer = 'type_spec'
            if Reply[c_id]['spec'] in 'AZ':
                backer = 'spec'
            button.add(types.InlineKeyboardButton(text='⬅️Назад', callback_data=Reply[c_id][backer]))
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Введіть кількість дефектів:', reply_markup=button)
        elif (call.data in Sheet[skl_hum(Reply[c_id], 'project_all')][Reply[c_id]['type']][Reply[c_id]['device']]) \
                and ('yes_photo' in Reply[c_id]):
            await call.answer('')
            Reply[c_id]['project'] = call.data
            delete = ['count_defects', 'non_liquidity', 'non_liquidity_board', 'non_liquidity_mult', '3_5_10',
                      'coments', 'type_defect', 'yes_coments_information']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            backer = Reply[c_id]['device']
            button = types.InlineKeyboardMarkup(row_width=1)
            ...
            await call.answer('')
            Reply[c_id]['type_defect'] = call.data
            ...
            button = Inline_Keyboard.inline_c2(stop, 'project', Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
            ...
            await call.answer('')
            Reply[c_id]['3_5_10'] = Sheet['3_5_10'][call.data]
            delete = ['count_defects', 'non_liquidity', 'non_liquidity_board', 'non_liquidity_mult', 'coments', 'yes_coments_information']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            if 'coments' in Reply[c_id]:
                del Reply[c_id]['coments']
            post = assembler_message.assembling_3_5_10(Reply[c_id])
            button.add(types.InlineKeyboardButton(text='Так', callback_data='yes_3_5_10'),
                       types.InlineKeyboardButton(text='⬅️Назад', callback_data=Reply[c_id]['type_defect']))
            Reply[c_id]['med'] = Inline_Keyboard.media(Reply[c_id], post)
            await bot.send_media_group(call.message.chat.id, media=Reply[c_id]['med'])
            ...
            await call.answer('')
            Reply[c_id]['information'] = call.data
            ...
            await call.answer('')
            Reply[c_id]['yes_coments_information'] = call.data
            delete = ['count_defects', 'non_liquidity', 'non_liquidity_board', 'non_liquidity_mult', 'coments']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            ...
            await call.answer('')
            post = assembler_message.assembling_information_defects(Reply[c_id])
            ...
            await call.answer('')
            Reply[c_id]['non_liquidity_mult'] = None
            ...
            backer = 'spec' if Reply[c_id]['stack_defects'] in '' else 'back_post'
            ...
            await bot.send_message(call.message.chat.id, text=f"{Reply[c_id]['stack_defects']}\n", reply_markup=button)
        elif call.data in 'back_amount':
            await call.answer('')
            b = Reply[c_id]['stack_defects'].split('Дефект:')
            b = b[:-1]
            Reply[c_id]['stack_defects'] = 'Дефект:'.join(b)
            Reply[c_id]['writer_stack'] = Reply[c_id]['writer_stack'][:-1]
            Reply[c_id]['amount'] = None
            delete = ['count_defects', 'non_liquidity', 'non_liquidity_board', 'coments', 'yes_coments_information']
            Inline_Keyboard.deleter_key(delete, Reply[c_id])
            button.add(types.InlineKeyboardButton(text='⬅️ Hазад', callback_data=Reply[c_id]['defects']))
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='Введіть кількість плат з дефектом:', reply_markup=button)
        elif call.data in 'non_count_sheet':
            await call.answer('')
            writer.writer(Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
            ...
            post = assembler_message.non_count_data(Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=post)
            await bot.send_message(call.message.chat.id, text='Перевірте чи вірно сформовані данні?', reply_markup=button)
        elif 'sheet_count_data' in call.data:
            await call.answer('')
            writer.writer_non_defect(Reply[c_id])
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                        text='✅ Данні успішно внесені в таблицю.\nЩоб Зробити новий запис натисніть /start')
...
async def photo(message):
    global Reply, mq, Sheet
    m_id = Inline_Keyboard.func_message(message)[0]
    if message.chat.id == m_id:
        Sheet = checker()
        if str(message.chat.id) in Sheet['access_id']:
            if 'yes_photo' in Reply[m_id]:
                if mq == '':
                    Reply[m_id]['photos'] = []
                    Reply[m_id]['videos'] = []
                    Reply[m_id]['med'] = []
                    Reply[m_id]['stack_defects'] = ''
                    Reply[m_id]['writer_stack'] = []
                try:
                    Reply[message.chat.id]['photos'].append(message.photo[0].file_id)
                except (KeyError, IndexError):
                    pass
                try:
                    Reply[message.chat.id]['videos'].append(message.video.file_id)
                except AttributeError:
                    pass
                line = [i for i in Sheet[skl_hum(Reply[m_id], 'lines')]]
                button = Inline_Keyboard.inline_c2(line, 'yes_photo', Reply[m_id])
                if mq == '':
    ...
    if message.chat.id == message.from_user.id:
        if str(message.from_user.id) in Sheet['access_id']:
            ...
            try:
                ...
                if ('3_5_10' in Reply[m_id]) and (Reply[m_id]['3_5_10'] in 'Коментар'):
                    Reply[m_id]['3_5_10'] = Sheet['3_5_10'][Reply[m_id]['3_5_10']]
                    Reply[m_id]['coments'] = text_message
                    post = assembler_message.assembling_3_5_10(Reply[m_id])
                    button.add(types.InlineKeyboardButton(text='Так', callback_data='yes_3_5_10'),
                               types.InlineKeyboardButton(text='⬅️Назад',
                                                          callback_data=Reply[m_id]['type_defect']))
                    Reply[m_id]['med'] = Inline_Keyboard.media(Reply[m_id], post)
                    await bot.send_media_group(message.chat.id, media=Reply[m_id]['med'])
                    await bot.send_message(message.chat.id, text='Перевірте чи вірно сформовані данні',
                                           reply_markup=button)
                elif ('information' in Reply[m_id]) and ('amount' not in Reply[m_id]):
                    if message.text.isdigit():
                        Reply[m_id]['amount'] = text_message
                        button.add(types.InlineKeyboardButton(text='Так', callback_data='coments_information'),
                                   types.InlineKeyboardButton(text='Ні', callback_data='no_coment_information'))
                        button.add(types.InlineKeyboardButton(text='⬅️Назад', callback_data='information_of_defect'))
                ...
                elif 'yes_coments_information' in Reply[m_id]:
                    Reply[m_id]['coments'] = text_message
                    post = assembler_message.assembling_information_defects(Reply[m_id])
                    button.add(types.InlineKeyboardButton(text='Так', callback_data='yes_information_of_defect'),
                               types.InlineKeyboardButton(text='⬅️Назад',
                    ...
                    await bot.send_message(message.chat.id, text='Перевірте чи вірно сформовані данні',
                                           reply_markup=button)
                elif ('yes_amount_data' in Reply[m_id]) and ('yes_count_amount_data' not in Reply[m_id]):
                    if message.text.isdigit():
                        Reply[m_id]['yes_count_amount_data'] = text_message
                        button.add(types.InlineKeyboardButton(text='Так', callback_data='no_amount_data'),
                                   types.InlineKeyboardButton(text='Ні', callback_data='no_count_data'))
                        button.add(types.InlineKeyboardButton(text='⬅️ Назад', callback_data='yes_amount_data'))
                        ...
                        post = assembler_message.input_validation_defect
                        await bot.send_message(message.chat.id, text=post)
                elif ('side' in Reply[m_id]) and ('non_liquidity' not in Reply[m_id]):
                    if message.text.isdigit():
                        Reply[m_id]['count_defects'] = text_message
                        button.add(
                            types.InlineKeyboardButton(text='Додати дефект', callback_data='NOK_defect'),
                            types.InlineKeyboardButton(text='Наступний крок ➡️', callback_data='non_liquidity')
                        )
                        button.add(types.InlineKeyboardButton(text='⬅️ Назад', callback_data='back_amount'))
                        Reply[m_id]['stack_defects'] += assembler_message.assembling_defect(Reply[m_id])
                        await bot.send_message(message.chat.id, text=f"{Reply[m_id]['stack_defects']}\n",
                                               reply_markup=button)
                    else:
                        post = assembler_message.input_validation_defect
                        await bot.send_message(message.chat.id, text=post)
                elif ('non_liquidity' in Reply[m_id]) and ('non_liquidity_mult' not in Reply[m_id]):
                    if message.text.isdigit():
                        Reply[m_id]['non_liquidity_board'] = text_message
                        Reply[m_id]['non_liquidity_mult'] = None
                        ...
                        post = assembler_message.input_validation_defect
                        await bot.send_message(message.chat.id, text=post)
                elif 'non_liquidity_mult' in Reply[m_id]:
                    if message.text.isdigit():
                        Reply[m_id]['non_liquidity_mult'] = text_message
                        post = assembler_message.non_count_data(Reply[m_id])
